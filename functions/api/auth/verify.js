export async function onRequestPost(context) {
  try {
    ...
    // 4. Create Session
    const user = await context.env.DB.prepare("SELECT id, email, first_name, last_name, full_name, role, phone_number, phone, default_delivery_zone, restricted_from_cod FROM users WHERE email = ?").bind(email).first();
    if (!user) {
